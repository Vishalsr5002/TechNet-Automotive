...
export default function Home() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!localStorage.getItem(`favorites_${matchedUser.email}`)) {
      localStorage.setItem(
        `favorites_${matchedUser.email}`,
        JSON.stringify([])
      );
    }
    if (!localStorage.getItem(`history_${matchedUser.email}`)) {
      localStorage.setItem(
        `history_${matchedUser.email}`,
        JSON.stringify([])
      );
    }
  ...
  return (
    <>
      <title>TechNet Automotive</title>
      <div className="relative min-h-screen">
        ...
        <section className="bg-white text-gray-800 py-20 px-8">
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {[
              { title: "Engine System", video: "/engine.mp4", desc: "Complete engine diagnostics and performance monitoring." },
              { title: "Brake System", video: "/brake.mp4", desc: "Advanced braking inspection and repair management." },
              { title: "Fluid Maintenance", video: "/fluid.mp4", desc: "Smart tracking of all fluid replacement schedules." },
            ].map((item, index) => (
              <div
                key={index}
                className="group p-10 rounded-xl shadow-lg border-b-4 border-blue-300 bg-white text-center transition-all duration-500 hover:-translate-y-4 hover:shadow-2xl"
              >
                <video
                  src={item.video}
                  muted
        ...
        <section className="bg-gray-100 text-gray-800 py-20 px-8">
          ...
          <div className="grid md:grid-cols-4 gap-8 text-center">
            {[
              ...
              <div
                key={index}
                className="p-6 border-l-4 border-blue-400 bg-white shadow rounded-lg hover:shadow-xl transition"
              >
                {feature}
              </div>
            ))}
        ...
        <footer className="bg-black text-white py-6 text-center">
          <p>© 2026 TechNet Automotive. All Rights Reserved.</p>
        </footer>
